...
//   // بيانات إحصائية
//   const stats = [
//     { title: "Courses completed", value: userCourses.filter(c => c.status === 'completed').length, icon: "📚" },
//     { title: "Total points gained", value: "250", icon: "⭐" },
//     { title: "Courses in progress", value: userCourses.filter(c => c.status === 'inProgress').length, icon: "📈" },
//     { title: "Tasks finished", value: "05", icon: "✅" }
//   ];
...
const DashboardPage = ({ user, userCourses = [], userPoints, setUserPoints }) => {
  const [activePeriod, setActivePeriod] = useState('Daily');
  const [courseIndex, setCourseIndex] = useState(0);
  const [points, setPoints] = useState(userPoints || 250);

  // تحديث النقاط عند تغييرها من المكون الأب
  useEffect(() => {
  ...
  const stats = [
    ...
    { title: "Total points gained", value: points.toString(), icon: "⭐" },
